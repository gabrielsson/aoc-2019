...
public class Day7 {
    ...
    public static Object part2(List<Integer> listOfRows) {
        return Generator.permutation(5, 6, 7, 8, 9).simple().stream()
                .mapToInt(phases -> findLargestThrust(listOfRows, phases)).max().orElse(-1);

    }

    private static Integer findLargestThrust(List<Integer> program, List<Integer> phases) {
        try {
            ThreadPoolExecutor executor = (ThreadPoolExecutor) Executors.newFixedThreadPool(5);
            List<BlockingQueue<Integer>> wires = IntStream.rangeClosed(0, phases.size())
